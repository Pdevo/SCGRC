...
print("connessione")
rednet.open(modem)
rednet.broadcast("avvio_segnale_"..n)
sleep(1)
print("funzionamento")
...
local s = segnale[stato]
local t = segnale[tempo]
 ...
 if s == "temporizzato" then
  ...
  segnale[stato] = s
  segnale[tempo] = t
  rednet.broadcast("aggiornamento_segnale_"..n.."_"..s)
 end
  ...
  if words[2] == "segnale" then
   ...
   if tonumber(words[3]) == n then
    ...
    if words[4] == "temporizzato" then
	 ...
	 segnale[stato] = s
	 segnale[tempo] = t
     rednet.broadcast("aggiornamento_segnale_"..n.."_"..s.."_predefinito")
	 salva_tabella(segnale,a)
     sleep(t)
	 local s = "inattivo"
	 local t = nil
	 ...
	 segnale[stato] = s
	 segnale[tempo] = t
     rednet.broadcast("aggiornamento_segnale_"..n.."_"..s.."_"..t)
	 salva_tabella(segnale,a)
     sleep(t)
	 local s = "inattivo"
	 local t = nil
 ...
 if words[1] == "sistemi" then
  ...
  if words[2] == "segnale" then
   ...
   if tonumber(words[3]) == n then
    ...
    rednet.broadcast("spegnimento_segnale_"..n)
    os.shutdown()
    end
    if words[4] == "riavvio" then
    rednet.broadcast("riavvio_segnale_"..n)
    os.reboot()
    end
